...
use units::{dimension::DIMENSIONLESS, parser::parse_unit, unit::Unit};

use errors::{OdysseyErrors, Result};
use crate::ecospold2::parse::{ids_from_str, EcoSpold};
    ...
    for (id, data) in processes.iter() {
        ...
        let item = InventoryItem {
            ...
            unit: parse_unit(&product.unit.clone())
                .unwrap_or(Unit { dimension: DIMENSIONLESS, scale_to_si: 0., substance: None }),
        };
        res.insert(id, item);
    }
    res
